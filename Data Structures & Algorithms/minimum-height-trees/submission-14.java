public class Solution {
    ...
    public List<Integer> findMinHeightTrees(int n, int[][] edges) {
        ...
        var path = dfsPathToTarget(deepestFirst[0], -1, deepestSecond[0]);
        var size = path.size();
        if (size != 0) {
    ...
    List<Integer> dfsPathToTarget(int node, int parent, int target) {
        ...
        for (var conn : adjList[node]) {
            if (conn != parent) {
                var list = dfsPathToTarget(conn, node, target);
                if (list.size() > 0) {
                    result = list;
                    break;
                }
